...
public class Product {
  ...
  public static final String NUMBER = "73167176531330624919225119674426574742355349194934"
  ...
  private static BigInteger multiplyDigits(String digitFeed) {
    int size = digitFeed.length();
    BigInteger product = BigInteger.ZERO;
    if (size > 0) {
      product = BigInteger.ONE;
      for (int i = 0; i < digitFeed.length(); i++) {
        product = product.multiply(new BigInteger(digitFeed.substring(i, i + 1)));
      }
    }
    return product;
  }
  ...
  public static void main(String[] args) {
    System.out.println(greatestProduct(NUMBER, 13));
  }
